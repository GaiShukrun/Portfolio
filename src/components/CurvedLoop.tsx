...
const CurvedLoop: React.FC<CurvedLoopProps> = ({
  ...
  // Create a circular path that matches the profile image (550px diameter)
  const radius = 275; // Half of 550px
  const centerX = 275;
  const centerY = 275;
  const pathD = `M ${centerX + radius} ${centerY} A ${radius} ${radius} 0 1 1 ${centerX + radius - 0.1} ${centerY}`;
  ...
  return (
    ...
    >
      <svg className="curved-loop-svg" viewBox="0 0 550 550">
        <text
          ref={measureRef}
          xmlSpace="preserve"
